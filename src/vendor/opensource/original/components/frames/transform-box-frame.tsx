...
type CornerHandleProps = Readonly<{
  handleColor: string;
  className?: string;
}>;

function CornerHandle({ className, handleColor }: CornerHandleProps) {
  return (
    <span
      aria-hidden
      style={{ borderColor: handleColor }}
      className={cn(
        "pointer-events-none absolute z-20 box-border size-3.5 border bg-white shadow-[0_1px_0_rgba(0,0,0,0.05)]",
        className,
      )}
    />
  );
...
export type TransformBoxFrameProps = Readonly<
  {
    children?: ReactNode;
    width?: string | number;
...
export const TransformBoxFrame = forwardRef<
  HTMLDivElement,
  TransformBoxFrameProps
>(
  (
    {
      className,
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className={cn("relative p-1.75", hasFixedHeight && "h-full")}>
          <div
            className={cn(
          ...
          <FrameEdge
            ...
            className="top-1.75 right-1.75 left-1.75 border-t"
          />
          <FrameEdge
            lineColor={colors.line}
            className="top-1.75 right-1.75 bottom-1.75 border-r"
          />
          <FrameEdge
            lineColor={colors.line}
            className="right-1.75 bottom-1.75 left-1.75 border-b"
          />
          <FrameEdge
            lineColor={colors.line}
            className="top-1.75 bottom-1.75 left-1.75 border-l"
          />
...
TransformBoxFrame.displayName = "TransformBoxFrame";
