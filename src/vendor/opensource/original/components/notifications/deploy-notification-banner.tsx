...
export type DeployNotificationBannerProps = Readonly<
  {
    appName?: string;
    project?: string;
    branch?: string;
...
// Deploy alert — iOS grid, dark app tile, short status copy.
export const DeployNotificationBanner = forwardRef<
  HTMLOutputElement,
  DeployNotificationBannerProps
>(
  (
    {
      className,
      ...
      branch = "main",
      duration = "42s",
      showTriggerLabel = "Show deploy alert",
      onDismiss,
      onShow,
      ...props
    },
    ...
    return (
      <output
        ref={ref}
        data-slot="deploy-notification-banner"
        data-phase={phase}
        className={cn(
      ...
      >
        ...
        <div className="grid grid-cols-[2.375rem_minmax(0,1fr)] items-start gap-x-3 gap-y-1 px-3.5 py-3 pr-9">
          <div className="row-span-2 mt-0.5 flex size-9.5 shrink-0 items-center justify-center rounded-[0.625rem] bg-neutral-900 shadow-sm ring-1 ring-black/2">
            <svg
              aria-hidden
              viewBox="0 0 76 65"
          ...
          <p className="col-start-2 line-clamp-2 text-[13px] leading-[1.38] text-neutral-700">
            ...
            <span className="text-neutral-500"> is live on </span>
            <span className="font-medium text-neutral-800">production</span>
...
DeployNotificationBanner.displayName = "DeployNotificationBanner";
